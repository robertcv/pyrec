...
class CapMF(MatrixFactorization):
    ...
    def fit(self, data: UIRData):
        ...
        train_select = np.arange(len(train_u))
        ...
        p_i = np.zeros(self.data.n)
        for u in np.arange(self.data.n):
            p_i[u] = len(np.unique(self.data.indexed_data.items[self.data.indexed_data.users == u])) / self.data.m
        ...
        for iteration in range(self.max_iteration):
            ...
            for i, j in zip(train_u[train_select], train_i[train_select]):
                ...
                expected_usage = np.sum(p_i[:, np.newaxis] * sigmoid(np.dot(self.P, self.Q.T)), axis=0)
                cl = self.inv.counts - expected_usage
                _r_ij = self._pred_vec(i, np.arange(self.data.m))
                tmp = sigmoid(-cl) * p_i[i] * sigmoid(_r_ij) * sigmoid(-_r_ij)
                ...
                expected_usage = np.sum(p_i * sigmoid(self._pred_vec(np.arange(self.data.n), j)))
                cl = self.inv.counts[j] - expected_usage
                _r_ij = self._pred_vec(np.arange(self.data.n), j)
                tmp = p_i * sigmoid(_r_ij) * sigmoid(-_r_ij)
                ...
                c_loss = np.mean(self.inv.counts[validation_i] - np.sum(p_i[validation_u] * sigmoid(self._pred_vec(validation_u, validation_i))))
